...
#extract latencies
result_shell = subprocess.run(f"python extract_latency.py --input {args.input}", shell=True, capture_output=True, text=True)
print(result_shell.stdout)

#replace GEMM_1 with GEMV_1 in the first row of the csv
csv_file = f"latencies_{args.input}.csv"
rows = []
...
#plot speedup
result_shell = subprocess.run(f"python plot.py --input latencies_{args.input}.csv", shell=True, capture_output=True, text=True)
