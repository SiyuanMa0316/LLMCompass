...
for mn_size in 2048 8192 32768; do
    for k_size in 2048 8192 32768; do
        echo "Testing M=${mn_size}, K=${k_size}, N=${mn_size}"
        python -m experiments.test_gemm_ddr5_multiprocess --config "$config" --M "$mn_size" --K "$k_size" --N "$mn_size" --precision "$precision" --roofline
    done
done
cd -
