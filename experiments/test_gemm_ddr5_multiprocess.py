...
parser.add_argument("--roofline", action='store_true', help="Print roofline model")
args = parser.parse_args()
M = args.M
K = args.K
N = args.N
model = Matmul(data_type=data_type_dict[args.precision])
_ = model(
    Tensor([M, K], data_type_dict[args.precision]),
    Tensor([K, N], data_type_dict[args.precision]),
)
...
def run_simulation(x, rank_count, col_count, arr_count, device_count, row_count):
    ...
    local_model = Matmul(data_type=data_type_dict[args.precision])
    _ = local_model(
        Tensor([M, K], data_type_dict[args.precision]),
        Tensor([K, N], data_type_dict[args.precision]),
    )
...
if __name__ == "__main__":  # required for Windows
    ...
    with ProcessPoolExecutor(max_workers=8) as executor:  # tune max_workers
        if not args.dse:
            ...
            print(f"Tiling utilization: {model.stats.tiling_utilization}")
            print(f"Column utilization: {model.stats.simd_utilization}")
            print(f"Capacity utilization: {model.stats.capacity_utilization}")
            print(f"GEMM {M}x{K}x{N} latency: {latency}s")
            print(f"simulated latency: GEMM_{M}x{K}x{N} {latency}")
            if args.roofline:
                latency_roofline = model.roofline_model(simdram)
                print(f"GEMM roofline latency: {latency_roofline}ms")
        else:
            for x in ["x4", "x8", "x16"]:
                if x == "x4":
